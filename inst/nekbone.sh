...
if [ ! -f $ROOTDIR/$BM/test/nek_mgrid/nekbone ]; then
	...
	if [ -z $1 ]; then
		...
		sed -i -e 's/^IFMPI=.*/IFMPI=false/' -e "s#-ipo -xHost#-CcdRR8 -Cpp -Fixed -O3 -Kfast -KA64FX -KSVE -KARMV8_3_A -Kassume=noshortloop -Kassume=memory_bandwidth -Kassume=notime_saving_compilation -Knolargepage -I$ROOTDIR/dep/mpistub/include/mpistub#g" -e 's# -L${ADVISOR_2018_DIR}/lib64 -littnotify##g' -e 's# -mkl-static##g' -e "s# -mkl# -Wl,-rpath -Wl,$ROOTDIR/dep/mpistub/lib/mpistub -L$ROOTDIR/dep/mpistub/lib/mpistub -lmpi -SSL2BLAMP#g" $ROOTDIR/$BM/src/makefile.template
