...
if [ ! -f $ROOTDIR/$BM/test/nek_mgrid/nekbone ]; then
	...
	if [ -z $1 ]; then
		...
		sed -i -e 's/^IFMPI.*/IFMPI:=false/' -e "s#-ipo -xHost#-CcdRR8 -Cpp -Fixed -O3 -Kfast -KA64FX -KSVE -KARMV8_3_A -Kassume=noshortloop -Kassume=memory_bandwidth -Kassume=notime_saving_compilation -Knolargepage#g" -e 's# -L${ADVISOR_2018_DIR}/lib64 -littnotify##g' -e 's# -mkl-static##g' -e "s# -mkl# -SSL2BLAMP#g" $ROOTDIR/$BM/src/makefile.template
		...
		sed -i -e 's/^CC=.*/CC=fccpx/g' -e 's/^F77=.*/F77=frtpx/g' -e 's/^#IFMPI="f/IFMPI="f/g' -e "s#-ipo -xHost#-CcdRR8 -Cpp -Fixed -O3 -Kfast -KA64FX -KSVE -KARMV8_3_A -Kassume=noshortloop -Kassume=memory_bandwidth -Kassume=notime_saving_compilation -Knolargepage#g" -e 's# -mkl-static##g' -e "s# -mkl# -SSL2BLAMP#g" ./makenek
