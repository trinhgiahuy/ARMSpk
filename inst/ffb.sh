...
if [ ! -f $ROOTDIR/$BM/bin/les3x.mpi ]; then
	...
	if [ ! -f ./REVOCAP_Refiner-1.1.01/lib/x86_64-linux-intel/libRcapRefiner.a ]; then
		...
		if [[ "$1" = *"intel"* ]]; then
			...
			rm ./MakefileConfig.in; ln -s ./MakefileConfig.Kei ./MakefileConfig.in
			sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./MakefileConfig.in
			sed -i -e 's/FFLAGS = -Kfast/FFLAGS = -Nclang -Ofast -mcpu=a64fx+sve -fopenmp -Kopenmp -ffj-ocl -ffj-largepage -flto/g' -e 's/-Kfast/-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-largepage -flto/g' -e 's/ -Kauto//g' ./MakefileConfig.in
		elif [[ "$1" = *"gem5"* ]]; then
			rm ./MakefileConfig.in; ln -s ./MakefileConfig.Kei ./MakefileConfig.in
			sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./MakefileConfig.in
			#sed -i -e 's/FFLAGS = -Kfast/FFLAGS = -Nclang -Ofast -mcpu=a64fx+sve -fopenmp -Kopenmp -ffj-ocl -ffj-no-largepage -fno-lto/g' -e 's/-Kfast/-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-no-largepage -fno-lto/g' -e 's/ -Kauto//g' ./MakefileConfig.in
			...
			rm ./MakefileConfig.in; ln -s ./MakefileConfig.Kei ./MakefileConfig.in
			sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./MakefileConfig.in
			sed -i -e 's/FFLAGS = -Kfast/FFLAGS = -mcpu=a64fx+sve -mtune=a64fx+sve -fopenmp -Kopenmp -Kfast,ocl,largepage,lto/g' -e "s#-Kfast.*#-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=full -fuse-ld=lld -L$(readlink -f $(dirname $(which mpifcc))/../lib64) -Wl,-rpath=$(readlink -f $(dirname $(which clang))/../lib)#g" ./MakefileConfig.in
		fi
		make
		if [[ "$1" = *"gnu"* ]]; then
	...
	if [[ "$1" = *"intel"* ]]; then
		...
		sed -i -e "s#/opt/klocal#$ROOTDIR/$BM/src/metis-5.1.0#g" ./make_setting
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./make_setting
		sed -i -e 's/^DEFINE += -DNO_REFINER/#DEFINE += -DNO_REFINER/g' -e "s#\$(HOME)/opt/REVOCAP_Refiner#$ROOTDIR/$BM/src/REVOCAP_Refiner-1.1.01#g" -e "s#REFINER)/lib #REFINER)/lib/kei #" -e 's/^FLAGS /FFLAGS /g' -e "s#REFINER)/include#REFINER)/Refiner#g" -e 's/-Kvisimpact,ocl -Qt/-Nclang -mcpu=a64fx+sve -fopenmp -Kopenmp -Kfast,ocl,nolargepage,nolto/g' -e 's/-Kvisimpact,ocl/-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-no-largepage/g' ./make_setting
