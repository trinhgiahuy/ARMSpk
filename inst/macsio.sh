...
if [ ! -f $ROOTDIR/$BM/macsio/macsio ]; then
	...
	if [ ! -f $ROOTDIR/dep/silo-4.10.2/bin/silofile ]; then
		...
		if [[ "$1" = *"intel"* ]]; then
			...
			./configure --prefix=`pwd` CC=gcc CFLAGS="-O2 -march=native" \
				CXX=g++ CXXFLAGS="-O2 -march=native" \
				FC=gfortran FCFLAGS="-O2 -march=native" \
				F77=gfortran FFLAGS="-O2 -march=native"
		elif [[ "$1" = *"fujitrad"* ]]; then
			...
			./configure --prefix=`pwd` CC=clang CFLAGS="-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin" \
				CXX=clang++ CXXFLAGS="-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin" \
				FC=frt FCFLAGS="-O2 -Nclang -mcpu=a64fx+sve -Kocl,largepage,lto" \
				F77=frt FFLAGS="-O2 -Nclang -mcpu=a64fx+sve -Kocl,largepage,lto" \
		fi
		make install
	...
	if [[ "$1" = *"intel"* ]]; then
		...
		cmake -DCMAKE_BUILD_TYPE=release \
			...
			-DCMAKE_CXX_COMPILER=`which mpiFCC` -DCMAKE_CXX_FLAGS="-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-largepage -flto" \
			-DCMAKE_INSTALL_PREFIX=../ -DWITH_JSON-CWX_PREFIX=../../dep/json-cwx -DWITH_SILO_PREFIX=../../dep/silo-4.10.2 ..
	elif [[ "$1" = *"gem5"* ]]; then
		cmake -DCMAKE_BUILD_TYPE=release \
		...
		cmake -DCMAKE_BUILD_TYPE=release \
			-DCMAKE_C_COMPILER=`which mpifcc` -DCMAKE_C_FLAGS="-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin" \
			-DCMAKE_CXX_COMPILER=`which mpiFCC` -DCMAKE_CXX_FLAGS="-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin" \
