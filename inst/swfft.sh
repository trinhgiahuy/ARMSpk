...
if [ ! -f $ROOTDIR/$BM/build.openmp/TestFDfft ]; then
	...
	if [[ "$1" = *"intel"* ]]; then
		...
		sed -i -e 's/?= mpicc/?= mpifcc/g' -e 's/?= mpicxx/?= mpiFCC/g' -e 's/?= mpif90/?= mpifrt/g' -e 's/-ipo -xHost -cpp/-mcpu=a64fx+sve -mtune=a64fx+sve -fopenmp -Kfast,ocl,largepage,lto -cpp/g' -e 's/-ipo -xHost/-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=full/g' -e "s# -I\${ADVISOR_2018_DIR}/include##g" -e "s# -L\${ADVISOR_2018_DIR}/lib64 -littnotify# -fuse-ld=lld -L$(readlink -f $(dirname $(which mpifcc))/../lib64) -Wl,-rpath=$(readlink -f $(dirname $(which clang))/../lib)#g"  ./GNUmakefile
