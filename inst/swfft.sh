...
if [ ! -f $ROOTDIR/$BM/build.openmp/TestFDfft ]; then
	...
	if [ ! -f $ROOTDIR/$BM/fftw/bin/fftw-wisdom ]; then
		...
		if [[ "$1" = *"intel"* ]]; then
			...
			./configure --host=aarch64-unknown-linux-gnu --build=x84_64-unknown-linux-gnu \
				--prefix=`pwd`/../fftw --disable-mpi --enable-openmp --disable-fortran CC=fcc
			make -j CFLAGS="-Nclang -Ofast -mcpu=a64fx+sve -ffj-ocl -ffj-largepage -flto"
		elif [[ "$1" = *"gem5"* ]]; then
			./configure --host=aarch64-unknown-linux-gnu --build=x84_64-unknown-linux-gnu \
				--prefix=`pwd`/../fftw --disable-mpi --enable-openmp --disable-fortran CC=fcc
			...
			make -j CFLAGS="-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin -fuse-ld=lld -L$(readlink -f $(dirname $(which mpifcc))/../lib64) -Wl,-rpath=$(readlink -f $(dirname $(which clang))/../lib)"
		fi
		make install
		cd $ROOTDIR/$BM/
	fi
