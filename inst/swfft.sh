...
if [ ! -f $ROOTDIR/$BM/build.openmp/TestFDfft ]; then
	...
	if [ -z $1 ]; then
		...
		sed -i -e 's/?= mpicc/?= fccpx/g' -e 's/?= mpicxx/?= FCCpx/g' -e 's/?= mpif90/?= frtpx/g' -e 's/-ipo -xHost//g' -e "s# -I\${ADVISOR_2018_DIR}/include# -I$ROOTDIR/dep/mpistub/include/mpistub#g" -e "s# -L\${ADVISOR_2018_DIR}/lib64 -littnotify# -Wl,-rpath -Wl,$ROOTDIR/dep/mpistub/lib/mpistub -L$ROOTDIR/dep/mpistub/lib/mpistub -lmpi -lmpifort#g" -e 's#DFFT_MPI_FC) $(DFFT_MPI_FFLAGS) -o#DFFT_MPI_CXX) $(DFFT_MPI_CXXFLAGS) -o#g' -e 's/DFFT_MPI_FLDFLAGS ?=.*/DFFT_MPI_FLDFLAGS ?= -lfjc++ -lfjc++abi -lfjdemgl/g' -e "s# -lm# -L$ROOTDIR/$BM -Bstatic -lm#g" ./GNUmakefile
