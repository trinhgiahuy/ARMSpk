...
if [ ! -f $ROOTDIR/$BM/build/bin/xhpcg ]; then
	...
	if [ -z $1 ]; then
		...
		../configure MPI_GCC_OMP
		sed -i -e 's/^CXX .*=.*/CXX = FCCpx/g' -e 's/-O3/-Nclang -Ofast -ffj-no-largepage -ffj-ocl -mllvm -polly/g' -e 's/-ftree-vectorizer-verbose=0//g' -e 's/^HPCG_OPTS .*=.*/HPCG_OPTS = -DHPCG_NO_MPI/g' ./setup/Make.MPI_GCC_OMP
		for FILE in `/usr/bin/grep 'include.*ittnotify' -r .. | cut -d':' -f1 | sort -u`; do sed -i -e 's/.*include.*ittnotify\.h.*/#define __itt_resume()\n#define __itt_pause()\n#define __SSC_MARK(hex)/' $FILE; done
	fi
	make
	cd $ROOTDIR
