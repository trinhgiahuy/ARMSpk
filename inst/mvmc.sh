...
if [ ! -f $ROOTDIR/$BM/src/vmc.out ] || [ "x`ls -s $ROOTDIR/$BM/src/vmc.out | awk '{print $1}'`" = "x0" ]; then
	...
	if [[ "$1" = *"intel"* ]]; then
		...
		cp ./Makefile_kei ./Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./Makefile_intel
		sed -i -e 's/-Kfast.*/-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-largepage -flto/g' ./Makefile_intel
		cp ./pfapack/Makefile_kei ./pfapack/Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./pfapack/Makefile_intel
		sed -i -e 's/-Kfast/-Nclang -mcpu=a64fx+sve -fopenmp -Kfast,ocl,largepage,lto/g' ./pfapack/Makefile_intel
		cp ./sfmt/Makefile_kei ./sfmt/Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./sfmt/Makefile_intel
		sed -i -e 's/-Kfast.*/-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-largepage -flto/g' ./sfmt/Makefile_intel
	elif [[ "$1" = *"gem5"* ]]; then
		...
		cd scalapack-2.0.2
		sed -e 's/mpif90/frt/g' -e 's/mpicc/fcc/g' -e "s#-O3#-Nclang -Kfast,openmp,ocl,nolargepage,nolto -I$ROOTDIR/dep/mpistub/include/mpistub#" -e "s#^LIBS .*#LIBS = -SSL2BLAMP -Wl,-rpath=$ROOTDIR/dep/mpistub/lib/mpistub -L$ROOTDIR/dep/mpistub/lib/mpistub -lmpi#" SLmake.inc.example > SLmake.inc
		make lib -j $(nproc); make lib
		cd $ROOTDIR/$BM/src
		cp ./Makefile_kei ./Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./Makefile_intel
		sed -i -e 's/-Kfast.*/-Nclang -Ofast -mcpu=a64fx+sve -fopenmp -ffj-ocl -ffj-no-largepage -fno-lto/g' -e 's/ = mpi/ = /g' -e "s#^CFLAGS = #CFLAGS = -I$ROOTDIR/dep/mpistub/include/mpistub #g" -e "s#^LIB = #LIB = $ROOTDIR/$BM/src/scalapack-2.0.2/libscalapack.a -Wl,-rpath=$ROOTDIR/dep/mpistub/lib/mpistub -L$ROOTDIR/dep/mpistub/lib/mpistub -lmpi #g" ./Makefile_intel
		cp ./pfapack/Makefile_kei ./pfapack/Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./pfapack/Makefile_intel
		...
		cp ./Makefile_kei ./Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./Makefile_intel
		sed -i -e 's/-Kfast.*/-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin/g' ./Makefile_intel
		cp ./pfapack/Makefile_kei ./pfapack/Makefile_intel
		sed -i -E 's/(fcc|FCC|frt)px/\1/g' ./pfapack/Makefile_intel
		sed -i -e 's/-Kfast/-Nclang -mcpu=a64fx+sve -fopenmp -Kfast,ocl,largepage,lto/g' ./pfapack/Makefile_intel
		cp ./sfmt/Makefile_kei ./sfmt/Makefile_intel
