...
if [ ! -f $ROOTDIR/$BM/bin/Linux_Intel64/xhpl ]; then
	mkdir -p $ROOTDIR/$BM/
	cd $ROOTDIR/$BM/
	if [ ! -f ./hpl-2.2.tar.gz ]; then wget http://www.netlib.org/benchmark/hpl/hpl-2.2.tar.gz; fi
	...
	if [ -z $1 ]; then
		...
		sed -i -e 's/mpiicc/fccpx/' -e 's/-ipo -xHost/-Kfast -Knolargepage/g' -e 's/ -z noexecstack -z relro -z now//g' -e 's/ -Wall//g' -e "s# -I\${ADVISOR_2018_DIR}/include# -I$ROOTDIR/dep/mpistub/include/mpistub#g" -e "s# -L\${ADVISOR_2018_DIR}/lib64 -littnotify# -Wl,-rpath -Wl,$ROOTDIR/dep/mpistub/lib/mpistub -L$ROOTDIR/dep/mpistub/lib/mpistub -lmpi#g" -e "s#\$(HOME)/hpl#$ROOTDIR/$BM#g" -e 's/ -ansi-alias -i-static//g' -e 's/ -nocompchk//g' -e 's/ -mt_mpi//g' -e 's/ -I$(LAinc)//g' -e "s# \$(LAlib)# -L$ROOTDIR/$BM/ -SSL2BLAMP#g" ./Make.Linux_Intel64
