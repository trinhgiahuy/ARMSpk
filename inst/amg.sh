...
if [ ! -f $ROOTDIR/$BM/test/amg ]; then
	...
	if [ -z $1 ]; then
		...
		sed -i -e 's/^CC =.*/CC = fccpx/g' -e 's/ -DTIMER_USE_MPI//g' -e 's/-ipo -xHost/-DHYPRE_SEQUENTIAL=1 -Nclang -Ofast -ffj-no-largepage -ffj-ocl -mllvm -polly -flto/g' ./Makefile.include
		sed -i -e 's# -I${ADVISOR_2018_DIR}/include##g' -e 's# -L${ADVISOR_2018_DIR}/lib64 -littnotify##g' ./Makefile.include
