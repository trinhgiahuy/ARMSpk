...
if [ ! -f $ROOTDIR/$BM/test/amg ]; then
	...
	if [[ "$1" = *"intel"* ]]; then
		...
		sed -i -e 's/^CC =.*/CC = mpifcc/g' -e 's/-ipo -xHost/-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=full/g' ./Makefile.include
