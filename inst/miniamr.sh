...
if [ ! -f $ROOTDIR/$BM/ref/ma.x ]; then
	...
	for SUB in ref openmp; do
		...
		if [[ "$1" = *"intel"* ]]; then
			...
			sed -i -e 's/^CC .*=.*/CC = mpifcc/' -e 's/^LD .*=.*/LD = mpifcc/' -e 's/-ipo -xHost/-Ofast -ffast-math -mcpu=a64fx -mtune=a64fx -fopenmp -mllvm -polly -mllvm -polly-vectorizer=polly -flto=thin -fcommon/g' -e 's/-qopenmp/-fopenmp/g' -e 's# -I${ADVISOR_2018_DIR}/include##g' -e "s# -L\${ADVISOR_2018_DIR}/lib64 -littnotify# -fuse-ld=lld -L$(readlink -f $(dirname $(which mpifcc))/../lib64) -Wl,-rpath=$(readlink -f $(dirname $(which clang))/../lib)#g" ./Makefile
