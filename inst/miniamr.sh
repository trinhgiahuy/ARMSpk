...
if [ ! -f $ROOTDIR/$BM/ref/ma.x ]; then
	...
	for SUB in ref openmp; do
		...
		if [ -z $1 ]; then
			...
			sed -i -e 's/^CC .*=.*/CC = mpifccpx/' -e 's/^LD .*=.*/LD = mpifccpx/' -e 's/-ipo -xHost/-Nclang -Ofast -ffj-ocl -mllvm -polly -flto/g' -e 's/-qopenmp/-fopenmp/g' -e "s# -I\${ADVISOR_2018_DIR}/include##g" -e "s# -L\${ADVISOR_2018_DIR}/lib64 -littnotify# -flto#g" ./Makefile
			...
			sed -i -e 's/^CC .*=.*/CC = fccpx/' -e 's/^LD .*=.*/LD = fccpx/' -e 's/ -lm / /g' -e 's/-ipo -xHost/-Nclang -Ofast -ffj-no-largepage -ffj-ocl -mllvm -polly/g' -e 's/-qopenmp/-fopenmp/g' -e "s# -I\${ADVISOR_2018_DIR}/include# -I$ROOTDIR/dep/mpistub/include/mpistub#g" -e "s# -L\${ADVISOR_2018_DIR}/lib64 -littnotify# -Wl,-rpath -Wl,$ROOTDIR/dep/mpistub/lib/mpistub -L$ROOTDIR/dep/mpistub/lib/mpistub -lmpi#g" ./Makefile
