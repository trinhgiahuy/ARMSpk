...
if [ -n "${XEONHOST}" ]; then
	...
	export TESTCONF="1|64 1|72 1|128 1|144 1|192 1|256 1|288
			 ...
			 256|1
			 288|1"
	export BESTCONF="96|2"
elif [ -n "${FUJIHOST}" ] || [ -n "${RFX7HOST}" ]; then
	export BINARY="./optimize_mp_${FUJIHOST}${RFX7HOST}/sw4lite"
	...
	if   [ -f ${ROOTDIR}/${APPDIR}/optimize_mp_fugaku/sw4lite ]; then
		export BINARY="./optimize_mp_fugaku/sw4lite"
	elif [ -f ${ROOTDIR}/${APPDIR}/optimize_mp_fx700/sw4lite ]; then
		export BINARY="./optimize_mp_fx700/sw4lite"
	else
		echo "ERR: missing sw4lite; check conf/sw4lite.sh; abort;"; exit 1;
	fi
