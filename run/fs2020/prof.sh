...
if [[ $HOSTNAME = *"${XEONHOST}"* ]]; then
	...
	MPIEXECOPT+=" -x OMP_PROC_BIND=close"
fi
...
LOGDIR="$ROOTDIR/log/`hostname -s`/profrun/fs2020"
mkdir -p $LOGDIR
cd $APPDIR
for BEST in $BESTCONF; do
	...
	MPIEXECOPT+=" -x OMP_NUM_PARALELL=$NumOMP -x FLIB_FASTOMP=FALSE -x FLIB_CNTL_BARRIER_ERR=FALSE"
	for BMconf in $BINARYS; do
		...
		if [ "x$RUNSDE" = "xyes" ]; then
			...
			mpiexec $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI bash -c "$SDE $BINARY $INPUT" >> $LOG 2>&1
