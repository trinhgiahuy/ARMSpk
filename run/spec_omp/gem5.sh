...
LOGDIR="$ROOTDIR/log/`hostname -s`/gem5run/spec_omp"
mkdir -p $LOGDIR
cd $APPDIR
...
EOF
	...
	bash -c "source ./shrc; $SPECCMD --iterations=1 --size=$SIZE --threads=$NumOMP --define COMP=$COMP --define RESDIR=${LOG}_stat $BM" >> $LOG 2>&1 &
done
wait
cd $ROOTDIR
