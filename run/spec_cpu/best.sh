...
for BENCH in $BINARY; do
	...
	for NumOMP in $BESTCONF; do
		echo -e "=== runing $BENCH ===\nsource ./shrc; $SPECCMD --size=$SIZE --threads=$NumOMP --define COMP=$COMP --define RESDIR=0 $BM" >> $LOG 2>&1
		START="`date +%s.%N`"
		bash -c "source ./shrc; timeout --kill-after=30s $MAXTIME $SPECCMD --size=$SIZE --threads=$NumOMP --define COMP=$COMP --define RESDIR=0 $BM" >> $LOG 2>&1
		ENDED="`date +%s.%N`"
		echo "Total running time: `echo \"$ENDED - $START\" | bc -l`" >> $LOG 2>&1
		REPORT="`find $ROOTDIR/$APPDIR/result -type f -name '*.log' | tail -1`"
		cat $REPORT >> $LOG 2>&1
		/bin/grep 'Run Reported' $REPORT >> $LOG 2>&1
		echo "Best $BENCH run: " "`/bin/grep 'Run Reported' $REPORT | cut -d'(' -f2 | cut -d')' -f1 | cut -d' ' -f3 | sort -g | head -1`"
	done
done
cd $ROOTDIR
