...
for BENCH in $BINARY; do
	...
	for NumOMP in $BESTCONF; do
		if [ "x$RUNSDE" = "xyes" ]; then
			...
			START="`date +%s.%N`"
			bash -c "export PATH=$SDEPATH:$PATH; source ./shrc; $SPECCMD --size=$SIZE --threads=$NumOMP --define COMP=sde --define RESDIR=$LOGDIR/${BM}_${NumMPI}_${NumOMP}_sde $BM" >> $LOG 2>&1
			ENDED="`date +%s.%N`"
