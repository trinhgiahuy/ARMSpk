...
for BEST in $BESTCONF $SCALCONF; do
	...
	if [ "x$RUNSDE" = "xyes" ]; then
		...
		echo "mpiexec $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI bash -c \"$SDE $BINARY\"" >> $LOG 2>&1
		for R in $TenRanks; do
			mpiexec $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI bash -c "if [[ \$PMIX_RANK = ${R} ]]; then $SDE $BINARY; else $BINARY; fi" >> $LOG 2>&1
	...
	if [ "x$RUNPCM" = "xyes" ]; then
		...
		for PCM in $PCMB; do
			...
			PCM+=" 360000 -- "
			echo "$PCM mpiexec $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI $BINARY" >> $LOG 2>&1
			$PCM mpiexec $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI $BINARY >> $LOG 2>&1
			cat ./msg.pe00000 >> $LOG 2>&1
		done
	fi
	if [ "x$RUNVTUNE" = "xyes" ]; then
		for VTO in $VTAO; do
			...
			echo "mpiexec -gtool \"amplxe-cl -collect $VTO $VTRO\" $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI $BINARY" >> $LOG 2>&1
			mpiexec -gtool "amplxe-cl -collect $VTO $VTRO" $MPIEXECOPT --map-by slot:pe=$ProcElem -x OMP_NUM_THREADS=$NumOMP -n $NumMPI $BINARY >> $LOG 2>&1
