...
for BEST in $BESTCONF; do
	...
	if [ "x$RUNSDE" = "xyes" ]; then
		...
		for R in `seq 0 $((NumMPI-1))`; do
			mpiexec $MPIEXECOPT -genv OMP_NUM_THREADS=$NumOMP -n $NumMPI bash -c "if [[ \$MPI_LOCALRANKID = ${R} ]]; then $SDE $BINARYYY $INPUT; else $BINARYYY $INPUT; fi" >> $LOG 2>&1
		done
	...
	if [ "x$RUNVTUNE" = "xyes" ]; then
		for VTO in $VTAO; do
			...
			echo "mpiexec -gtool \"amplxe-cl -collect $VTO $VTRO\" $MPIEXECOPT -genv OMP_NUM_THREADS=$NumOMP -n $NumMPI $BINARYYY $INPUT" >> $LOG 2>&1
			mpiexec -gtool "amplxe-cl -collect $VTO $VTRO" $MPIEXECOPT -genv OMP_NUM_THREADS=$NumOMP -n $NumMPI $BINARYYY $INPUT >> $LOG 2>&1
